...
function App() {
  ...
  // Signal names (wire aliases) are loaded once; renames go through the
  // canonical signals.rename operation and update the local map.
  useEffect(() => {
    void opCall<{ signals: { wire: number; name: string }[] }>("signals.list").then((result) => {
      const names: Record<number, string> = {};
      for (const signal of result.signals) names[signal.wire] = signal.name;
      setSignalNames(names);
    }).catch(() => { /* names default to wire labels */ });
  }, []);
