...
{selected !== null && interpreters.length > 0 && <section className="decoders-panel"><h2>Decoders</h2><div className="decode-config"><label className="baud">baud / bitrate<input type="number" value={decodeRate} onChange={(event) => setDecodeRate(Number(event.target.value))} /></label><label className="baud inv"><input type="checkbox" checked={isoInverse} onChange={(event) => setIsoInverse(event.target.checked)} />inverse</label></div><div className="decoder-list">{interpreters.map((interp) => <div className="decoder-item" key={interp.id}><div className="decoder-head"><b>{interp.name}</b><small className="kind">{interp.type.toUpperCase()}</small><small className="wires">{interp.wires.map(channelLabel).join(",")}</small><button className="mini decode" onClick={() => void decodeInterpreter(interp)}>Decode</button></div>{decodedFrames[interp.id] !== undefined && <div className="decoded-frames">{decodedFrames[interp.id]!.length === 0 ? <span className="muted">no frames</span> : <>{decodedFrames[interp.id]!.slice(0, 200).map((frame, index) => <span key={index} className="frame">{frame.text}</span>)}{decodedFrames[interp.id]!.length > 200 && <span className="muted">+{decodedFrames[interp.id]!.length - 200} more</span>}</>}</div>}</div>)}</div></section>}
