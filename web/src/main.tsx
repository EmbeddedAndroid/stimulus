...
function App() {
  ...
  const decodeInterpreter = async (interp: Interpreter) => {
    try {
      const params: Record<string, unknown> = { id: interp.id };
      if (interp.type === "uart") params.baud = uartBaud;
      const result = await opCall<{ frames: { text: string }[] }>("interp.frames", params);
...
{selected !== null && interpreters.length > 0 && <section className="decoders-panel"><h2>Decoders</h2><label className="baud">UART baud<input type="number" value={uartBaud} onChange={(event) => setUartBaud(Number(event.target.value))} /></label><div className="decoder-list">{interpreters.map((interp) => <div className="decoder-item" key={interp.id}><div className="decoder-head"><b>{interp.name}</b><small className="kind">{interp.type.toUpperCase()}</small><small className="wires">{interp.wires.map(channelLabel).join(",")}</small><button className="mini decode" onClick={() => void decodeInterpreter(interp)}>Decode</button></div>{decodedFrames[interp.id] !== undefined && <div className="decoded-frames">{decodedFrames[interp.id]!.length === 0 ? <span className="muted">no frames</span> : <>{decodedFrames[interp.id]!.slice(0, 200).map((frame, index) => <span key={index} className="frame">{frame.text}</span>)}{decodedFrames[interp.id]!.length > 200 && <span className="muted">+{decodedFrames[interp.id]!.length - 200} more</span>}</>}</div>}</div>)}</div></section>}
