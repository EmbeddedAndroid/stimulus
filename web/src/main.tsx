...
const NAV_CHANNEL = 0;
// Protocol decoders that can be created and pointed at captured channels.
const DECODER_KINDS = ["uart", "spi", "i2c", "can", "onewire", "parallel", "iso7816"] as const;
...
function App() {
  ...
  const [decodedFrames, setDecodedFrames] = useState<Record<string, { text: string }[]>>({});
  // Why a decode returned no frames (flat/unacquired wire, or an active-but-
  // frameless window), keyed by interpreter id; shown in place of "no frames".
  const [decodeReason, setDecodeReason] = useState<Record<string, string>>({});
  // Rate for the protocols that need one (UART/ISO baud, CAN bitrate) and the
  // ISO 7816 convention; ignored by the self-clocked protocols.
  const [decodeRate, setDecodeRate] = useState(9600);
  ...
  const decodeInterpreter = async (interp: Interpreter) => {
    try {
      ...
      const result = await opCall<{ frames: { text: string }[]; reason?: string | null }>("interp.frames", { id: interp.id, ...rateParams, inverse: isoInverse });
      setDecodedFrames((current) => ({ ...current, [interp.id]: result.frames }));
      setDecodeReason((current) => ({ ...current, [interp.id]: result.reason ?? "" }));
    } catch {
      setDecodedFrames((current) => ({ ...current, [interp.id]: [] }));
      setDecodeReason((current) => ({ ...current, [interp.id]: "decode request failed" }));
    }
  };
  const [newDecoderKind, setNewDecoderKind] = useState<string>("uart");
  const [newDecoderName, setNewDecoderName] = useState("");
  const [newDecoderWires, setNewDecoderWires] = useState("");
  const createInterpreter = async () => {
    const wires = parseWires(newDecoderWires);
    if (wires.length === 0) return;
    try {
      await opCall("interp.create", { type: newDecoderKind, name: newDecoderName.trim() || undefined, wires });
      setNewDecoderName(""); setNewDecoderWires("");
      await refresh();
    } catch { /* validation reported by the daemon */ }
  };
  const removeInterpreter = async (interpId: string) => {
    try {
      await opCall("interp.remove", { id: interpId });
      setDecodedFrames((current) => { const next = { ...current }; delete next[interpId]; return next; });
      await refresh();
    } catch { /* ignore */ }
  };
  ...
  useEffect(() => { setCursors({}); setView(null); setDecodedFrames({}); }, [selectedId]);
  ...
  // Load the state list (an RLE table of the capture) when that view is shown.
  useEffect(() => {
    if (mainView !== "statelist" || capId === null) return;
    let cancelled = false;
    void (async () => {
      try {
        const result = await opCall<{ config: { columns: string[] }; rows: StatelistRow[] }>("statelist.get", { capture_id: capId });
        if (!cancelled) setStatelist({ columns: result.config.columns, rows: result.rows });
      } catch { if (!cancelled) setStatelist({ columns: [], rows: [] }); }
    })();
    return () => { cancelled = true; };
  }, [mainView, capId]);
...
{selected !== null && <section className="decoders-panel"><h2>Decoders</h2><div className="decode-config"><label className="baud" title="0 = auto-detect the line rate from the waveform">baud / bitrate<input type="number" min="0" value={decodeRate} onChange={(event) => setDecodeRate(Number(event.target.value))} /></label><label className="baud inv"><input type="checkbox" checked={isoInverse} onChange={(event) => setIsoInverse(event.target.checked)} />inverse</label></div>{interpreters.length === 0 ? <p className="muted">No decoders yet. Add one and point it at captured channels.</p> : <div className="decoder-list">{interpreters.map((interp) => <div className="decoder-item" key={interp.id}><div className="decoder-head"><b>{interp.name}</b><small className="kind">{interp.type.toUpperCase()}</small><small className="wires">{interp.wires.map(channelLabel).join(",")}</small><button className="mini decode" onClick={() => void decodeInterpreter(interp)}>Decode</button><button className="mini" title={`Remove ${interp.name}`} onClick={() => void removeInterpreter(interp.id)}>✕</button></div>{decodedFrames[interp.id] !== undefined && <div className="decoded-frames">{decodedFrames[interp.id]!.length === 0 ? <span className="muted">{decodeReason[interp.id] || "no frames"}</span> : <>{decodedFrames[interp.id]!.slice(0, 200).map((frame, index) => <span key={index} className="frame">{frame.text}</span>)}{decodedFrames[interp.id]!.length > 200 && <span className="muted">+{decodedFrames[interp.id]!.length - 200} more</span>}</>}</div>}</div>)}</div>}<div className="decoder-add"><select aria-label="Decoder type" value={newDecoderKind} onChange={(event) => setNewDecoderKind(event.target.value)}>{DECODER_KINDS.map((kind) => <option key={kind} value={kind}>{kind.toUpperCase()}</option>)}</select><input aria-label="Decoder name" placeholder="Name" value={newDecoderName} onChange={(event) => setNewDecoderName(event.target.value)} /><input aria-label="Decoder wires" placeholder="D0,D1" value={newDecoderWires} onChange={(event) => setNewDecoderWires(event.target.value)} /><button onClick={() => void createInterpreter()} disabled={busy || newDecoderWires.trim() === ""}>Add</button></div></section>}
...
function statelistCell(row: StatelistRow, column: string): string | number {
  switch (column) {
    case "sample": return row.sample;
    case "time": return formatTime(row.time_s);
    case "data": return row.formatted_data;
